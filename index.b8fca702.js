...
class Game {
    ...
    mergeLeft(row) {
        ...
        for(let i = 0; i < nonZeroArray.length; i++){
            if (i < nonZeroArray.length - 1 && nonZeroArray[i] === nonZeroArray[i + 1]) {
                const merged = nonZeroArray[i] * 2;
                newRow[index] = merged;
                this.score += merged;
                i++;
            } else newRow[index] = nonZeroArray[i];
            index++;
        }
        return newRow;
    }
    ...
    canMove() {
        for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
            ...
            if (col < 3 && cell === this.board[row][col + 1]) return true;
            if (row < 3 && cell === this.board[row + 1][col]) return true;
        }
        return false;
    }
}
